...
void Lab4::RenderScene() {
  modelMatrix["box1"] = glm::mat4(1);
  modelMatrix["box1"] *= transform3D::Translate(-2.5f, 0.5f, -1.5f);
  modelMatrix["box1"] *=
      transform3D::Translate(translateX, translateY, translateZ);
  RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix["box1"]);

  modelMatrix["box2"] = glm::mat4(1);
  modelMatrix["box2"] *= transform3D::Translate(0.0f, 0.5f, -1.5f);
  modelMatrix["box2"] *= transform3D::Scale(scaleX, scaleY, scaleZ);
  RenderMesh(meshes["box"], shaders["Simple"], modelMatrix["box2"]);

  modelMatrix["box3"] = glm::mat4(1);
  modelMatrix["box3"] *= transform3D::Translate(2.5f, 0.5f, -1.5f);
  modelMatrix["box3"] *= transform3D::Translate(-2.5, 0, 0);
  modelMatrix["box3"] *= transform3D::RotateOY(angularBonus);
  modelMatrix["box3"] *= transform3D::Translate(2.5, 0, 0);
  modelMatrix["box3"] *= transform3D::RotateOX(angularStepOX);
  modelMatrix["box3"] *= transform3D::RotateOY(angularStepOY);
  modelMatrix["box3"] *= transform3D::RotateOZ(angularStepOZ);
  RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix["box3"]);
}
...
void Lab4::OnKeyPress(int key, int mods) {
  ...
  if (key == GLFW_KEY_I) {
    ...
    miniViewportArea.height *= 0.9f;
    miniViewportArea.width *= 0.9f;
  }
}
