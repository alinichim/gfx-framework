...
void Lab6::Init() {
    ...
    // normal
    {
        ...
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab6", "shaders", "VertexShader.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab6", "shaders", "FragmentShader.glsl"),
                          GL_FRAGMENT_SHADER);
...
Mesh *Lab6::CreateMesh(const char *name,
...
void Lab6::Update(float deltaTimeSeconds) {
    {
        ...
        modelMatrix =
            glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 0));
...
void Lab6::FrameEnd() { DrawCoordinateSystem(); }

void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader,
