...
void Lab5::Init() {
  ...
  fov = 40;
  projectionMatrix =
      glm::perspective(RADIANS(60), window->props.aspectRatio, z_near, z_far);
}
...
void Lab5::Update(float deltaTimeSeconds) {
  ...
  if (projectionType == 'P') {
    projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio,
                                        z_near, z_far);
...
void Lab5::OnKeyPress(int key, int mods) {
  ...
  if (key == GLFW_KEY_P) {
    projectionType = 'P';
    // projectionMatrix = glm::perspective(RADIANS(fov),
    // window->props.aspectRatio, z_near, z_far); fov
  }
  if (key == GLFW_KEY_O) {
    projectionType = 'O';
    // projectionMatrix = glm::ortho(left, right, bottom, top, z_near, z_far);
